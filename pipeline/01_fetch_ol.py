...
# ── Download helpers ───────────────────────────────────────────────────────────
def download_gz_stream(url: str, dest: Path, max_retries: int = 3, min_size_mb: float = 0):
    """Download a gzip file with retry logic, magic-byte and size validation."""
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            # Validate minimum size — a too-small file means the server truncated the response
            if min_size_mb > 0 and size_mb < min_size_mb:
                raise ValueError(
                    f"File too small: {size_mb:.1f} MB < {min_size_mb:.1f} MB minimum — "
                    f"server likely truncated the response"
                )
            return
...
def build_reading_counts(reading_log_gz: Path) -> dict:
    ...
    try:
        ...
        print(f"  Warning: reading-log gz ended prematurely — continuing with {len(counts):,} entries")
    return counts
...
def build_ratings(ratings_gz: Path) -> dict:
    ...
    try:
        ...
        print(f"  Warning: ratings gz ended prematurely — continuing with {len(ratings):,} entries")
    return ratings
...
def build_author_names(authors_gz: Path) -> dict:
    ...
    try:
        ...
        print(f"  Warning: authors gz ended prematurely — continuing with {len(names):,} names")
...
# ── Main stream ────────────────────────────────────────────────────────────────
def stream_works(works_gz: Path, ratings: dict, read_counts: dict, author_names: dict) -> list:
    candidates = []
    seen_norm: set = set()
    ...
    try:
        ...
        print(f"  Warning: works gz ended prematurely — continuing with {len(candidates):,} candidates so far")
...
def main():
    ...
    # Skip if candidates already exist (cache hit from previous run this month)
    if CANDIDATE_PATH.exists() and CANDIDATE_PATH.stat().st_size > 1_000_000:
        count = sum(1 for _ in open(CANDIDATE_PATH))
        print(f"  Candidates file already exists ({count:,} entries) — skipping downloads")
        return
    ...
    if not works_gz.exists():
        download_gz_stream(OL_WORKS_DUMP_URL, works_gz, min_size_mb=500)
    else:
        print(f"  Using cached {works_gz.name}")

    if not authors_gz.exists():
        download_gz_stream(OL_AUTHORS_DUMP_URL, authors_gz, min_size_mb=50)
    else:
        print(f"  Using cached {authors_gz.name}")

    if not ratings_gz.exists():
        download_gz_stream(OL_RATINGS_DUMP_URL, ratings_gz, min_size_mb=1)
    else:
        print(f"  Using cached {ratings_gz.name}")

    if not reading_gz.exists():
        download_gz_stream(OL_READING_LOG_URL, reading_gz, min_size_mb=20)
    else:
    ...
    # Stream works
    candidates = stream_works(works_gz, ratings, read_counts, author_names)
